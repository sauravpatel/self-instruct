...
def llama2_api_request(input, temperature = 0.0, logprobs = None, stop_sequences = []):
...
def make_requests(
    ...
    while retry_cnt <= retries:
        try:
            response = llama2_api_request(prompts,
                                          temperature = temperature,
                                          stop_sequences = stop_sequences)
            break
    ...
    if isinstance(prompts, list):
        ...
        for j, prompt in enumerate(prompts):
            print (f'Choices {json.dumps(response, indent=4)}')
            data = {
                "prompt": prompt,
